...
\item
  打开清华镜像站 Miniconda
  目录：\texttt{https://mirrors.tuna.tsinghua.edu.cn/anaconda/miniconda/}；
\item
  下载 \textbf{最新版}
...
\begin{quote}
若提示 channel 冲突，也可只保留 default 并在安装时加
\texttt{-c\ https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/main/}。
\end{quote}
...
\tightlist
\item
  打开 \texttt{https://code.visualstudio.com/}，下载 Windows
  安装包并安装（默认选项即可）；
\item
  安装完成后，打开命令行确认：
\end{enumerate}
...
\tightlist
\item
  网址：\texttt{https://www.overleaf.com/}
\item
...
\tightlist
\item
  网址：\texttt{https://www.loongtex.com/}
\item
...
\item
  打开
  \texttt{https://www.tug.org/texlive/acquire-netinstall.html}，下载
...
\begin{quote}
也可以直接用课程模板自带的
\texttt{.vscode/settings.json}（\texttt{教学资源/LaTeX模板/latex\_vscode\_template/}），拷贝进你的项目即可。
\end{quote}
...
安装 Git for Windows：\texttt{https://git-scm.com/}。四个核心命令：
