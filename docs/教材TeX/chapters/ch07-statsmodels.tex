...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={三种肥料的产量箱线图}]{figures/07-statsmodels/case1_fertilizer_anova.png}}
\caption{三种肥料的产量箱线图}
\end{figure}
...
\textbf{一句话人话}：房价 ≈ 一个``起步价'' + 每平米单价 × 面积 −
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={拟合值 vs 真实值 与 残差图}]{figures/07-statsmodels/case2_ols_multi.png}}
...
\textbf{一句话人话}：先问``序列稳不稳''（ADF），不稳就先差分（\texttt{d}=1）；再用``多少阶自相关/移动平均''（ACF/PACF
+ AIC）挑
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={ARIMA 选阶与残差诊断图}]{figures/07-statsmodels/case3_arima_diag.png}}
