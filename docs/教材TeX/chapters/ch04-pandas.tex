...
\textbf{一句话解释}：把两个带标签的 Series 相加时，Pandas
...
\textbf{一句话解释}：先把``空缺''和``离谱的数值''补好，再把一天一天的数值聚成一周一周，并画一条平滑的
3 天平均。
...
\textbf{一句话解释}：先把数据变干净，再按班级``算总和/平均''，最后把结果画成图；三步里每一步都不难，关键是选对
Pandas 方法。
