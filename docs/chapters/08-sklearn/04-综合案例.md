...
> 本章新增综合案例，把 01–03 的知识串成一条完整流水线：**特征缩放 → 分类 → 特征重要性 → 聚类 → 降维 → 可视化**。本综合案例按“案例卡 4”组织：目标 → 步骤 → 讲解 → 主要用法 / API → 常见错误 → 拓展练习。所有代码在 sklearn 1.6.1 下验证；配图由 `build/make_chap8_figures.py` 生成。
...
- 用 `StandardScaler` + `Pipeline` 完成“缩放 + 分类”，在训练/测试切分与 5 折交叉验证中都避免数据泄漏；
- 用随机森林 `feature_importances_` 找出区分品种的关键特征；
- 用 `KMeans` 做无监督聚类，并用 `silhouette_score` / `adjusted_rand_score` 评估聚类质量；
- 用 `PCA` 降到 2 维并可视化，比较“真实类别”与“KMeans 簇”在平面上的重合程度。
...
**一句话解释**：把 13 个红酒指标当成 13 维坐标，先缩放拉平尺度，再训练分类器、看哪些指标最重要，最后用 KMeans 分堆并用 PCA 投到平面上一眼看清。

1. 第 1 步打印 `wine.feature_names` 与 `X.shape`，先“看清数据”。13 个特征量纲不同，所以后面必须做缩放。
2. 第 2 步用 `Pipeline` 把 `StandardScaler` 和分类器串起来；`cross_val_score` 时每折都会重新 `fit` 缩放器，避免信息泄漏。5 折均值：LogisticRegression=0.9832、RandomForest=0.9778。
3. 第 3 步查看 `feature_importances_`：`flavanoids`（0.1945）、`color_intensity`（0.1730）、`alcohol`（0.1416）贡献最大，说明这几个指标最能区分品种。
4. 第 4 步 KMeans 聚类 `ARI=0.8975`，说明无监督分组与真实品种高度一致；轮廓系数 0.2849 说明簇间还有一定重叠。
5. 第 5 步 PCA 前两个主成分解释方差约 55%，2D 平面已能看出三团；按真实标签与按 KMeans 簇着色基本重合，说明这个数据在低维确实可分。

## 主要用法 / API

| 需求 | 写法 | 说明 |
| ---- | ---- | ---- |
| 缩放 + 分类 | `Pipeline([('sc', StandardScaler()), ('clf', LogisticRegression())])` | 训练/预测统一走流水线 |
| 交叉验证 | `cross_val_score(pipe, X, y, cv=5)` | 每折重新拟合 |
| 特征重要性 | `rf.feature_importances_` | 越大越重要 |
| 聚类评估 | `silhouette_score`、`adjusted_rand_score` | 内紧外分 / 与真实标签一致性 |
| 降维 | `PCA(n_components=2)` | 读 `explained_variance_ratio_` |

## 常见错误

- 在 `cross_val_score` 里把“缩放后的 X”传进去，而不是传 `Pipeline` → 数据泄漏。
...
- sklearn 组合与 Pipeline：[链接](https://scikit-learn.org/stable/modules/compose.html)
- sklearn 特征重要性：[链接](https://scikit-learn.org/stable/modules/ensemble.html#feature-importance)
- sklearn 聚类评估（ARI/silhouette）：[链接](https://scikit-learn.org/stable/modules/clustering.html#clustering-performance-evaluation)
