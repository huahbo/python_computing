...
> 本步依赖步骤 3 定义的 `fig`，可在画出报告图后直接另存一份 300 DPI 的 PDF，便于打印或插入文档。

```python
import os
os.makedirs('images', exist_ok=True)
fig.savefig('images/case_study_report.pdf', dpi=300)
print('pdf saved, dpi 300')
```
...
> 小提示：`savefig` 的 `dpi` 只影响输出文件的分辨率，不改变 `fig` 本身的 `dpi`；想要画布本身更精细，可在 `plt.subplots(figsize=..., dpi=...)` 里设置。
