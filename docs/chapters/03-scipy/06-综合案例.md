...
> 所有代码在 SciPy/NumPy/statsmodels 环境实际运行；配图由 `make_chap3_figures.py` 生成。
...
print(f"f1 估计 = {popt[3]:.3f}   f2 估计 = {popt[6]:.3f}")
```
...
- **固定输出格式**：第 1 个代码块开头用 `np.set_printoptions(precision=4, suppress=True)`，让后面所有 NumPy 数组打印统一到 4 位有效数字，跨机器/版本更稳；
- **统一浮点格式**：每个数值都用 `round(...,4)` 或 `:.4f`/`:.3f` 输出，避免直接打印完整浮点（如 `0.200908...`）造成对比困难；
- **容错初值与迭代上限**：`curve_fit` 用接近真值的 `p0`，并设 `maxfev=50000`，防止复杂模型“达到最大调用次数”而失败；
- **先补缺失再分析**：先用 `CubicSpline` 把 NaN 段补成连续曲线，再进行 FFT / 拟合 / 积分 / 检验，保证整条流水线输入一致。

## 延伸阅读

- SciPy 插值 / FFT / 优化官方文档见各节“官方文档/参考入口”。
- 信号处理完整示例：[链接](https://docs.scipy.org/doc/scipy/tutorial/fft.html)
- statsmodels 多重比较：[链接](https://www.statsmodels.org/stable/stats.html)
