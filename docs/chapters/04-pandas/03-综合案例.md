...
把本章知识串成一条完整流水线：**构造数据 → 清洗（缺失/重复/异常）→ groupby / pivot_table 统计 → matplotlib 可视化**，并学会为配图配置中文字体、让图表保存到 `images/` 目录。
...
**一句话解释**：先把数据变干净，再按班级“算总和/平均”，最后把结果画成图；三步里每一步都不难，关键是选对 Pandas 方法。

1. 构造数据：用 `rng = np.random.default_rng(42)` 固定随机种子，保证每次生成的“学生成绩”一样；`round(score, 1)` 让分数保留 1 位小数。
2. 清洗：`dropna(subset=['分数'])` 删掉缺分数的行；`drop_duplicates()` 删重复行；Q1/Q3/IQR 找出离群分数并用全体中位数替换。
3. 统计：`groupby('班级')['分数'].mean()` 得到各班平均分；`pivot_table(...)` 得到“班级 × 科目”矩阵，便于横向比较。
4. 可视化：`plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'DejaVu Sans']` 让中文正常显示；`ax.bar`、`ax.hist`、`ax.plot` 分别画柱状、直方图与折线；`fig.savefig('images/...png')` 把图存到 `images/`。
...
| 需求 | 写法 | 说明 |
| ---- | ---- | ---- |
| 固定随机种子 | `rng = np.random.default_rng(42)` | 结果可复现 |
| 去缺失行 | `df.dropna(subset=['分数'])` | 只按指定列判断 |
| 去重复行 | `df.drop_duplicates()` | 默认保留首次 |
| IQR 异常掩码 | `(df['分数'] < Q1-1.5*IQR) | (df['分数'] > Q3+1.5*IQR)` | 布尔筛选 |
| 分组平均 | `df.groupby('班级')['分数'].mean()` | 返回 Series |
| 透视表 | `df.pivot_table(values='分数', index='班级', columns='科目', aggfunc='mean')` | 行列交叉表格 |
| 中文绘图字体 | `plt.rcParams['font.sans-serif']=['Microsoft YaHei','SimHei','DejaVu Sans']` | 防止中文缺字 |
| 保存图片 | `fig.savefig('images/case_scores.png')` | 需先 `os.makedirs('images', exist_ok=True)` |

### 常见错误

- 未固定随机种子导致每次运行结果不同，课堂演示/作业对不上；
- `dropna()` 不加 `subset` 会把“任何一列有缺失”的行都删掉；
- IQR 的界限为 `[Q1-1.5*IQR, Q3+1.5*IQR]`，漏乘 `1.5` 会把正常值当异常；
- 在临时目录运行 `fig.savefig('images/...')` 时，若 `images/` 不存在会报 `FileNotFoundError`（先 `os.makedirs('images', exist_ok=True)`）；
- matplotlib 中文乱码是没配置 `font.sans-serif`，不是数据的问题。
...
- Pandas 分组与聚合文档：[链接](https://pandas.pydata.org/docs/user_guide/groupby.html)
- Pandas 透视表文档：[链接](https://pandas.pydata.org/docs/reference/api/pandas.pivot_table.html)
- Pandas 时间序列文档：[链接](https://pandas.pydata.org/docs/user_guide/timeseries.html)
