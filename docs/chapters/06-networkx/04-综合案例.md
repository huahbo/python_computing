...
> 本页即本章综合案例卡（案例卡 4）：把 01–03 的知识串起来。
...
| 需求 | 写法 | 说明 |
| ---- | ---- | ---- |
| 载入经典网络 | `nx.karate_club_graph()` | 34 节点 78 边 |
| 度分布 | `sorted([d for _, d in K.degree()], reverse=True)` | 长尾/无标度特征 |
| 全局指标 | `nx.average_clustering` / `nx.transitivity` / `nx.average_shortest_path_length` | 小世界效应 |
| 社区划分 | `nxcom.greedy_modularity_communities(K)` | 返回社区集合 |
| 模块度 | `modularity(K, comms)` | 划分质量 Q |
| 最短路径 | `nx.shortest_path(K, 0, 33)` | 无权最短路 |
| 可视化 | `nx.draw_networkx_*` + `spring_layout(K, seed=42)` | 固定布局便于复现 |

### 常见错误

1. 忘记导入社区子模块：社区函数在 `networkx.algorithms.community` 下，需要 `from networkx.algorithms import community as nxcom`。
2. 把模块度 Q 当成“越大越绝对正确”：Q 是相对指标，0.41 对 34 节点网络已算不错，但不同算法或 seed 可能得到不同社区数。
3. `spring_layout` 不传 `seed`：每次布局不同，截图/复现对不上。
4. 把“度分布降序图”误当成直方图：这里 x 轴是“按度排序的节点编号”，并不是度数分箱。

### 拓展 / 跟练

1. 分别计算 0 号与 33 号的度、介数中心性、接近中心性，比较两者谁更像“中心”。
2. 用 `nx.community.louvain_communities(K, seed=42)` 划分，比较社区个数与 Q 值。
3. 把“0→33 最短路径”推广成“所有点对平均最短路径”，并解释“六度分隔”。
4. 用 `nx.to_numpy_array(K)` 得到 34x34 邻接矩阵，验证对称；再交给 SciPy 做特征值（可选）。
