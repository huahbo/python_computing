...
- 练习：exercises/quiz.ipynb 第 6–8 题（审查题与任务卡题）；
- 上机：lab/lab.ipynb Part A（审查预置产出物）；
- 下一节：04 案例一（读懂并修复 NumPy 代码）。
