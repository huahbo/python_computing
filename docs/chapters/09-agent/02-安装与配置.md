...
> ⏱ **时效内容（基线 v0.1.5-rc.2 / 2026-09-19）**：本节涉及版本号、命令、界面与价格，是全书最容易过期的部分。遇到不一致时以官方文档为准；更新流程见 build/dsh_更新SOP.md。
...
| 卸载后旧版本还在 | 多个 Node 共存 | Windows 用 `where node`、macOS / Linux 用 `which -a node` 查看来源并清理 |
...
> 本章的维护机制：dsh_version.yaml 记录基线版本，build/check_dsh_release.py 定期比对官方版本，双周自动提醒（见 build/dsh_更新SOP.md）。
