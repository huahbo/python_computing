...
综合案例不只是“把三个方法跑一遍”，还应该**检验模型是否值得信任**。下面补两件事：① 对日负荷做 ADF 平稳性检验（定 `d` 的依据）；② 把前 120 天当训练、后 20 天当测试，分别用 ARIMA 与“温度+周末”回归做预测，用测试集 RMSE 比较谁更准。
...
adf_p = adfuller(load_s)[1]
print('load ADF p = %.3g' % adf_p)

tr = load_s.iloc[:120]
te = load_s.iloc[120:]
mtr = ARIMA(tr, order=(1, 1, 1)).fit()
fc = mtr.forecast(steps=len(te))
rmse_arima = float(np.sqrt(np.mean((te.values - fc.values)**2)))
print('ARIMA(1,1,1) test RMSE = %.3f' % rmse_arima)

x_tr = dfc[['temp', 'weekend']].iloc[:120]
y_tr = dfc['load'].iloc[:120]
x_te = dfc[['temp', 'weekend']].iloc[120:]
y_te = dfc['load'].iloc[120:]
mreg = sm.OLS(y_tr, sm.add_constant(x_tr)).fit()
pred_reg = mreg.predict(sm.add_constant(x_te))
rmse_reg = float(np.sqrt(np.mean((y_te.values - pred_reg.values)**2)))
print('OLS(temp,weekend) test RMSE = %.3f' % rmse_reg)
```
...
```text
load ADF p = 0.774
ARIMA(1,1,1) test RMSE = 29.412
OLS(temp,weekend) test RMSE = 7.808
```

**解读**：

- `load ADF p = 0.774 > 0.05`：日负荷序列非平稳，所以第 3 步用 `ARIMA(...,d=1)` 是合理的；若想用 `SARIMAX` 含外生温度，也应先差分。
- 测试集 RMSE 上，`OLS(temp, weekend)` 为 **7.81**，明显优于 `ARIMA(1,1,1)` 的 **29.41**：因为负荷主要受温度与周末影响，回归能“用上外生变量”，而纯 ARIMA 只能靠历史轨迹外推。
- 这不代表 ARIMA 没用：若没有温度等外生变量，或只关心短期的纯时间形态，ARIMA 是更好的基线；更复杂的 `SARIMAX`（把 `temp` 当外生变量）通常能进一步压小 RMSE。
