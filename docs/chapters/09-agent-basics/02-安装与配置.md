...
| 下载很慢或超时 | 网络问题 | 临时换用国内镜像（命令见下方代码块）；或按校网要求配置代理 |
...
> 提示：npm 下载慢时，可以临时换用国内镜像；企业网络或校园网如有代理，请按网络管理员要求配置。
...
![设置 → 模型：填入 API Key](./images/shot_settings_model.png)

> 图：设置 → 模型。把 API Key 粘贴到 DeepSeek 卡片的输入框并保存（截图：基线 v0.1.5-rc.2 / 2026-09-19）。
