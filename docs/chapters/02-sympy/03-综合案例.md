...
plt.fill_between(xs, ys, color='#9ec7e8', alpha=0.55, label=f'S = int x^2 dx = {float(area):.4f}')
plt.plot(xs, ys, color='#2f6fb3', lw=2.4, label='y = x^2')
...
print(f"{float(sp.N(numeric_deriv)):.12f}")
```
...
- SymPy 官方绘图（Plotting）模块：[链接](https://docs.sympy.org/latest/modules/plotting.html)
- Matplotlib 填充区域：[链接](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.fill_between.html)
- NumPy 数值求根（np.roots）：[链接](https://numpy.org/doc/stable/reference/generated/numpy.roots.html)