...
**一句话解释**：`solve` 爱把结果塞进一个列表里，`solveset` 更喜欢用集合表示，并且能告诉你“这个根只在复数里出现”这类信息——所以做题目时要看清你要的是“一个解”还是“所有解”，以及“是不是实数解”。
...
**一句话解释**：`lambdify` 把“带 x、y 的公式”变成一个“填数字就出结果”的普通函数，而且它能一口气处理整个 NumPy 数组，不用你写 for 循环。
